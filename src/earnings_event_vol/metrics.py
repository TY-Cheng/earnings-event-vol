...
def qlike_loss(actual: Sequence[float], forecast: Sequence[float]) -> float:
    ...
    f = np.maximum(np.asarray(forecast, dtype=float), EPSILON)
    ratio = y / f
...
def forecast_metrics(
    ...
    *,
    ...
    if baseline_col in frame.columns:
        baseline = frame.loc[clean.index, baseline_col].pipe(pd.to_numeric, errors="coerce")
        valid = np.isfinite(baseline)
        if bool(valid.any()):
            y = clean.loc[valid, target_col]
            f = clean.loc[valid, forecast_col]
            b = baseline.loc[valid]
            denominator = float(np.sum(np.square(y - b)))
            result["oos_r2_vs_ivar"] = (
                None
                if denominator <= EPSILON
                else float(1.0 - np.sum(np.square(y - f)) / denominator)
            )
    return result
...
def ranking_metrics(
    ...
    *,
    ...
    return {
        ...
        "brier": brier_score(outcome, probability),
...
def strategy_metrics(
    ...
    *,
    ...
    gross_pnl_col: str = "gross_proxy_pnl_usd",
    ...
    net = pd.to_numeric(out[net_pnl_col], errors="coerce").dropna()
    if net.empty:
        return {
            "n": 0,
            "gross_pnl_usd": None,
    ...
    gross = (
        pd.to_numeric(out[gross_pnl_col], errors="coerce").fillna(0.0)
...
def cost_sensitivity(
    ...
    *,
    ...
    if gross_pnl_col not in frame.columns or cost_col not in frame.columns:
        raise ValueError(f"frame must include {gross_pnl_col} and {cost_col}")
    rows: list[dict[str, float | int]] = []
    gross = pd.to_numeric(frame[gross_pnl_col], errors="coerce").fillna(0.0)
    cost = pd.to_numeric(frame[cost_col], errors="coerce").fillna(0.0)
...
def breakdown_metrics(
    ...
    *,
    ...
    for keys, group in frame.groupby(list(by), dropna=False):
        ...
        pnl = (
            strategy_metrics(group, net_pnl_col=net_pnl_col) if net_pnl_col in group.columns else {}
        )
        forecast = (
